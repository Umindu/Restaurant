...
public class ProductController implements Initializable{
    ...
    void saveProduct(ActionEvent event) {
        ...
        if (!proId.isEmpty() && !proName.isEmpty() && proCategory != "Select Category" && !proCost.isEmpty() && !proPrice.isEmpty()) {
            //save to database
            if (proIdTextField.isEditable()) {
                try {
                    Statement statement = DBConnect.connectToDB().createStatement();
                    statement.execute("insert into Products values ('"+proId+"', '"+proName+"', '"+proCategory+"', '"+proCost+"', '"+proDiscount+"', '"+proPrice+"', '"+proDescription+"')");
                ...
                try {
                    Statement statement = DBConnect.connectToDB().createStatement();
                    statement.execute("update Products set Name = '"+proName+"', Category = '"+proCategory+"', Cost = '"+proCost+"', Discount = '"+proDiscount+"', Price = '"+proPrice+"', Description = '"+proDescription+"' where ID = '"+proId+"'");
        ...
        try {
            ...
            while (resultSet.next()) {
                ...
                editBtn.setOnAction(e -> {
                    editProduct(new Product(id, name, category, cost, discount, price, description, actionBtnContainer));
                });
                deleteBtn.setOnAction(e ->{
                    deleteProduct(new Product(id, name, category, cost, discount, price, description, actionBtnContainer));
                });

                productList.add(new Product(id, name, category, cost, discount, price, description, actionBtnContainer));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
